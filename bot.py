...
TOKEN          = "MTQ4MDA2OTU1ODI2OTExNjQ0Nw.GndTQQ.nTClAKQBFdf-_NiG2tPv7B4bNrLRCgpNxdIGdU"
...
GITHUB_RAW = "https://raw.githubusercontent.com/talaferz12-rgb/legendary-octo-garbanzo/main/bot.py"
BOT_FILE   = __file__
...
def get_remote_hash():
    try:
        with urllib.request.urlopen(GITHUB_RAW, timeout=10) as r:
...
async def auto_update_loop():
    ...
    print("✅ نظام التحديث التلقائي يعمل")
    while not bot.is_closed():
        try:
            local_hash  = get_file_hash(BOT_FILE)
            remote_hash = get_remote_hash()
            if remote_hash and local_hash != remote_hash:
                print("🔄 تحديث جديد على GitHub — يتم التحديث...")
                with urllib.request.urlopen(GITHUB_RAW, timeout=10) as r:
                    new_code = r.read()
                with open(BOT_FILE, "wb") as f:
                    f.write(new_code)
                print("✅ تم تحديث الملف — يتم إعادة التشغيل...")
                import os, sys
...
async def manual_update(ctx):
    if not ctx.author.guild_permissions.administrator:
        return await ctx.send(embed=lucifer_embed("❌", "للأدمن فقط!", C_FIRE))
    await ctx.send(embed=lucifer_embed("🔄 جاري التحديث...", "يتم سحب آخر نسخة من GitHub", C_GOLD))
    try:
        local_hash  = get_file_hash(BOT_FILE)
        remote_hash = get_remote_hash()
        if remote_hash == local_hash:
            return await ctx.send(embed=lucifer_embed("✅ محدّث", "البوت على آخر نسخة بالفعل!", C_GOLD))
        with urllib.request.urlopen(GITHUB_RAW, timeout=10) as r:
            new_code = r.read()
        with open(BOT_FILE, "wb") as f:
            f.write(new_code)
        await ctx.send(embed=lucifer_embed("✅ تم التحديث!", "يتم إعادة تشغيل البوت...", C_GOLD))
        import os, sys
        os.execv(sys.executable, [sys.executable] + sys.argv)
    except Exception as e:
        await ctx.send(embed=lucifer_embed("❌ خطأ", str(e), C_FIRE))
